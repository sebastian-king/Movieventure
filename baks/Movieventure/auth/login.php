	...
	do {
		...
		if (!password_verify($_POST['password'], $r['password'])) {
			...
			if (@$_POST['remember-me'] == 1) {
				$expires = strtotime("+10 years"); // almost never?
			} else {
				$expires = 0;
			}
			...
			setcookie(COOKIE_PREFIX . "_SESSION_ID", $auth_session_id, $expires, '/', $root_domain, true, true);
			setcookie(COOKIE_PREFIX . "_SESSION_NAME", $auth_session_name, $expires, '/', $root_domain, true, true);
			setcookie(COOKIE_PREFIX . "_r", $r['username'], time()*2, '/auth/');
					...
					<form action="" method="POST">
						...
						<div class="row">
							<div class="col-xs-8 text-left checkbox">
								...
								<input type="checkbox" name="remember-me" value="1" <?php if (@$_POST['remember-me'] || @$_COOKIE[COOKIE_PREFIX . '_r'] == "true") { echo "checked='checked'"; } ?>> Remember me
								</label>
							</div>
							<div class="col-xs-4">
