<!--===================================================-->
    ...
    $(document).ready(function() {
	  ...
	  $(document).keypress(function(e) {
		...
		<div class="delay">\
			<hr>\
			<p style="font-style: italic;">If this takes more than a minute or two, you may check the status of our services <a href="https://status.movieventure.net/" target="_blank">here</a>.</p>\
			<p style="font-style: italic;">Or please e-mail us at <a href="mailto:help@movieventure.net">help@movieventure.net</a> for support.</p>\
		</div>\
	</div>').css({'background-image': 'none', 'background-color': 'rgba(18,56,88,0.75)'});
				}
			}).removeClass("active");
      	};
      ...
      websocket.onerror = function(evt) {
			...
			if (!/^#/.test(href)) { // it is just a hash change, no need to reload the ajax
				if (/^\/(me\/[^/]+|movie\/[^/]+|[^/]+)?$/.test(href)) {
					//if (window.location.pathname != href) {
						...
						var script_src = "/js" + href + ".js";
                        			script.src = script_src;
						var already_loaded = false;
						if ($('script[src="' + script_src + '"]').length == 0) {
                        				document.head.appendChild(script);
			                        	console.log('extra JS loaded: ', script.src);
						} else {
							already_loaded = true;
							console.log('JS already loaded: ', script.src);
						}
						var load_func = script_src.replace(/[\W]+/g,"_");
						...
						$.get(url, function(data) {
							...
							window.current_path = window.location.pathname;
							history_update_title(new_title);
							if (!already_loaded) {
								console.log('calling: ', load_func);
								var function_name = "nav_loaded" + load_func;
								var load_ext_script_timeout = 2000;
								var load_ext_script_iterator = 0;
								var load_ext_script_interval = setInterval(function() {
									if (isFunction(window[function_name])) {
										window["nav_loaded" + load_func]();
										clearInterval(load_ext_script_interval);
									} else {
										load_ext_script_iterator++;
										console.log(function_name + "() is not a function @ " + (load_ext_script_iterator * 10) + "/" + load_ext_script_timeout);
										if (load_ext_script_iterator * 10 > load_ext_script_timeout) {
											console.log("looking for function timed out @ " + load_ext_script_timeout);
											clearInterval(load_ext_script_interval);
										}
									}
								}, 10);
		...
		window.current_path = window.location.pathname;
		window.current_hash = window.location.hash;
		window.addEventListener('popstate', function(event) {
			console.log('popstate fired!');

			if (window.location.pathname == window.current_path && window.location.hash != window.current_hash) {
				console.log('just a hash change');
				window.current_hash = window.location.hash;
				return;
			} else {
				console.log(window.current_path, window.current_hash);
