...
if (auth()) {
	...
	function search_tpb($query, $category = NULL) {
		if (is_null($category)) {
			global $categories;
			$category = $categories[0];
		}
		//$search_results = shell_exec('wget -qO- https://thepiratebay.rocks/search/' . rawurlencode($query) . '/0/99/' . $category);
		$search_results = shell_exec('torsocks wget -qO- http://uj3wazyk5u4hnvtk.onion/search/' . $query . '/0/99/' . $category);
		$i = 0;
	...
	function parse_results($results, $query) {
		...
		foreach ($m[0] as $key => $title) {
			...
			// torrent magnet link
			preg_match('@<a href="magnet:(.+?)" title="@i', $title, $magnet);
			$magnet = "magnet:${magnet[1]}";

			// torrent username & user badges
			...
			// uploaded date & total size of torrent data
			...
			preg_match("@icon_comment.gif\" alt=\"This torrent has (\d+) comments.\"@i", $title, $comments);
	...
	function sort_results_by_quality($results, $search_query) {
		...
		// are more comments indicative of a more active, better torrents? or a bad one with complaints
	...
	$id_results = parse_results(search_tpb($search_query), $search_query);

	$search_query = "$title $release_year";
	$nid_results = parse_results(search_tpb($search_query), $search_query);
